...
\subsubsection*{Berechnung des Schlüsselaustausches mittels DH}
Alice
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    \item wählt zufällige, allgemein bekannte Primzahl $p$
    \item wählt Basisgenerator $g\in[1,p-1]$
    \item wählt zufälliger \textsl{private key} $a$
    \item erstellt damit ihren \textsl{public key} $A$
    \item sendet öffentlich $A, g, p$ an Bob
    \item Bob
    \begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
        \item erhält $A, g, p$ von Alice
        \item wählt zufälligen \textsl{private key} $b$
        \item erstellt mit Hilfe von erhaltenen $g, p$ seinen \textsl{public key} $B$
        \item generiert den \textsl{\textbf{secret key}} $S$
        \item sendet öffentlich $B$ an Alice
    \end{itemize}
    \item generiert ihrerseits mit Hilfe von $B$ den \textsl{\textbf{secret key}}
\end{itemize}
Um es mit Zahlen zu veranschaulichen:\\
$A=g^{\color{red}a}\ mod\ p$\\
$p=5, g=2, {\color{red}a=6}$\\
$A=4\equiv 2^{\color{red}6}\ (mod\ 5)$\\
Alice $(A, g, p) \rightarrow$ Bob\\
$B=g^{\color{red}b}\ mod\ p$\\
${\color{red}b=7}$\\
$B=3\equiv 2^{\color{red}7}\ (mod\ 5)$\\
$S_{secret\ key}=A^{\color{red}b}\equiv g^{{\color{red}a}\times {\color{red}b}}\ (mod\ p)$\\
$S_{secret\ key}=4\equiv 4^{\color{red}7}\equiv 2^{{\color{red}6}\times {\color{red}7}}\ (mod\ 5)$\\
Alice $\leftarrow (B)$ Bob\\
$S_{secret\ key}=B^{\color{red}a} \equiv g^{{\color{red}a}\times {\color{red}b}}\ (mod\ p)$\\
$S_{secret\ key}=4\equiv 3^{\color{red}6}\equiv 2^{{\color{red}6}\times {\color{red}7}}\ (mod\ 5)$\\
Denn $4^7\ mod\ 5=4$ und $3^6\ mod\ 5=4$. Natülich wird mit gigantischen Zahlen und Primzahlen gerechnet. Logisch sind in userem Rechenbeispiel als secret key nur 0 bis 4 möglich.
