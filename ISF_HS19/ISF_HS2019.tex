...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    \item \textbf{Identität:} "`Beim Menschen bezeichnet Identität die ihn kennzeichnende und als Individuum von anderen Menschen unterscheidende Eigentümlichkeit seines Wesens."'\cite{wiki}
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    ...
    \item \textbf{Datenschutz}: Schutz der Persönlichkeit und der Grundrechte von Personen, über die Daten bearbeitet werden
\end{itemize}
...
\begin{tabular}{|ll|}
    \hline
    Multiplikation (einfach):&$97\times83=8051$\\
...
\begin{figure}[H]
    ...
    \caption{Alice verschlüsselt mittels Bobs Public Key asymmetrisch}
    \label{pke}
    \end{center}
\end{figure}
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    ...
    \item Gemeinsamer \textsl{Secret Key} {\color{dunkelblau}{\color{red}$K$}$= g^{a\cdot b}\ \mathrm{mod}\ p$} ist nur Alice und Bob bekannt,\\ohne dass Alice $b$ kennt und Bob $a$
\end{itemize}
\begin{figure}[H]
    \begin{center}
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    ...
    \item Der Session-Key wird asymmetrisch mit Bobs Public-Key verschlüsselt
...
Alice vertraut der Authentizität von Bobs Public Key, durch direktes Überprüfen, normalerweise über den Fingerprint des Key's.
\begin{figure}[H]
    \begin{center}
...
\paragraph*{HTTP}\index{HTTP}Der Browser kommuniziert mit dem Webserver über das \textbf{Hypertext Transfer Protokoll (HTTP)}. HTTP besteht aus \textsl{Requests} und \textsl{Responses}.
...
\paragraph*{Häufigste Response-Codes}\index{HTTP!Response-Codes}\index{Response-Codes}
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    \item 200 OK
    \item 204 No Content
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    ...
    \item \textbf{Same Site} (wird nicht bei Cross-Domain-Aufrufen mitgesendet, z.B. `embedded' Link, Image)
\end{itemize}
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    ...
    \item Cookies als `Secure', `HttpOnly' oder `Same Site' mit so eingeschränkter Domain und Pfad wie möglich setzen
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    ...
    \item \textbf{Zusätzlich:} Same-Site-Attribut setzen
\end{itemize}
...
\paragraph*{Denial of Service}Syn-Nachrichten werden mit gespoofter IP gesendet. Syn-Acknowledgement-Nachrichten gehen nirgendwo hin.
Der Server wird überflutet mit Abfragen und kann diese nicht schneller abarbeiten als sie reinkommen.
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    ...
    \item \textbf{Sicherstellen, dass das System sich nicht selbst überlastet durch Freigeben nicht mehr verwendeter Ressourcen, vermeiden von unendlichen Loops etc.}
...
\paragraph*{Preisgabe Sensitiver Daten: Was ist das?} Angreifer stehlen Schlüssel, Passwörter, Geschäftsgeheimnisse,
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    ...
    \item ISO 27004: Information security management – Measurement (Ziele müssen messbar sein, z.B. Jahresziele beim Mitarbeitergespräch; Ende Periode kann überprüft werden, ob die Ziele erreicht wurden)
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    ...
    \item Baut auf der IT-Grundschutzvorgehensweise auf (BSI-Standard 100-2)
\end{itemize}
\begin{figure}[H]
    \begin{center}
...
\end{figure}
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    \item Die \textbf{Identifizierungsfunktion} hilft bei der Entwicklung eines organisatorischen Verständnisses für das Management des Cybersicherheitsrisikos für Systeme, Personen, Anlagen, Daten und Fähigkeiten
    \item Die \textbf{Schutzfunktion} unterstützt die Fähigkeit, die Auswirkungen potenzieller Cybersicherheitsereignisse zu begrenzen oder einzudämmen, und setzt Rahmenbedingungen der Schutzmassnahmen für die Bereitstellung kritischer Dienste
...
\subsection*{Sie kennen den Aufbau der IT-Grundschutz-Kataloge und deren Anwendungsweise}\index{Risiko-Management}\index{Grundschutz!IT-Grundschutz}\index{IT-Grundschutz}
\begin{figure}[H]
    \begin{center}
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    ...
    \item Der monetäre Wert von Assets muss genau bekannt sein.
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    ...
    \begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
        \item Die Einhaltung gesetzlicher und vertraglicher Pflichten sind stark gefährdet oder die Dienstleistungen werden verunmöglicht.
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    ...
    \item \textbf{Risiken übertragen (transferieren):}\\Überwälzung finanzieller Schäden auf Versicherungen, Outsourcer oder Benutzer eines Services.
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    ...
    \end{itemize}
    \item \textbf{Risk Identification}\index{Risiko-Management-Prozess!Risk Identification}\\Objekte, Bedrohungen,
    Schwachstellen und bereits existierende Massnahmen
    erfassen
    \begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
        \item Erfassung der Assets (Risiko-Objekte)
        \item vollständige Erfassung der Gefahrenquellen und das Aufsuchen bereits existierender Massnahmen
    ...
    \end{itemize}
    \item \textbf{Risk Evaluation}\index{Risiko-Management-Prozess!Risk Evaluation}\\Bewertung der identifizierten Risiken im
    ...
    \pagebreak
    \item \textbf{Risk Monitoring and Review}\index{Risiko-Management-Prozess!Risk Monitoring and Review}\\Prozess und Risiko-Situation bezüglich allfälliger Veränderungen überwachen
    \begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    ...
    \begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
        \item Standardisierte Massnahmen
...
\index{Grundschutz!IT-Grundschutz}\index{IT-Grundschutz}
\begin{figure}[H]
    \begin{center}
...
\paragraph*{Bemerkungen zu den IT-Grundschutz-Katalogen}\index{Grundschutz!IT-Grundschutz}\index{Grundschutz!Kataloge}\index{IT-Grundschutz}
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    ...
    \end{itemize}
    \item Netwerkplan \textsl{auswerten}
    \begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    \item Vererbung und Maximumprinzip berücksichtigen: Schutzbedarf bemisst sich am Schutzbedarf der IT-Systeme und der Informationen, welche im IT-Raum gelagert und verarbeitet werden
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    \item Nachbilden des erhobenen IT-Verbunds mit Hilfe der vorgegebenen IT-Grundschutz-Bausteine
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    ...
    \item Schichtweise werden diejenigen Bausteine ausgewählt, welche für die Sicherheit des IT-Verbunds notwendig sind
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    ...
    \item Mögliche Umsetzungsstatus
    \begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    ...
    Grundwerte vorliegt,
    \item kein geeigneter Baustein im Baustein-Katalog zu finden ist oder
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    \item Alle Mitarbeitenden und Lieferanten des Unternehmens sollten im Zusammenhang mit der von ihnen ausgeführten Tätigkeiten, bewusstseinsbildende Ausbildungen und Trainings ausführen, sowie regelmässige Updates von Bestimmungen und Verfahren erhalten.
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    ...
    \item Unsicherer Passwort recovery/reset Prozess
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    \item Absicherung eines Geheimnisses gegen Verrat durch einzelne Teilinformation
    \begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
        \item Einzelpersonen haben nur Teilinformationen
        \item Geheimnis kann aus Teilinformation rekonstruiert werden
    \end{itemize}
    \item Schutz der Gruppe gegen Blockade durch einzelne Teilgruppe
    \begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
        \item Geheimnis kann von Teilgruppe rekonstruiert werden
    \end{itemize}
    \item Sicherheitsschwelle abhängig von Bedeutung des Geheimnisses
\end{itemize}
...
\begin{multicols}{2}
    ...
    \paragraph*{Risiken}
    \begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
        \item Sicherheitsmängel beim Identity-Provider geben Zugang zum Service-Provider
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
	\item \textbf{Berechtigungen:} Mitarbeiter haben Zugriff (lesend und schreibend) auf Daten, die sie nicht benötigen, haben administrative Berechtigung an ihrem Arbeitsgerät oder Administratoren benutzen ihr Arbeitskennwort auch zur Systemverwaltung (Active Directory). Massnahmen fordern dazu, dass eine rigorose Einschränkung der Zugriffsberechtigungen durchgeführt wird sowie keine administrativen Berechtigungen für Benutzer. Jedoch starke Einschränkungen behindern die Arbeit.
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
	...
	\item \textbf{Mittelfristige Massnahmen(Schaden beheben):} Einsatz einer Task-Force (Notfallorganisation), Information an Kunden und Partner, Forensische Untersuchung der Infektion, Aufbau einer intakten Umgebung, Wiederherstellung der Daten (ab Backup), Nacharbeiten.
