...
\paragraph*{Verfügbarkeit}\index{Verfügbarkeit}\index{Grundziele Informationssicherheit}\index{Schutzziele}
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
...
\pagebreak
\subsubsection*{Sicherer Schlüsselaustausches mittels DH}\index{Schlüsselaustauschprotokoll}\index{Diffie-Hellman}
\paragraph*{Protokoll mit Diskreten Logarithmen über $\mathbb{Z}_p$\footnote{https://www.cs.uni-potsdam.de/ti/lehre/07-Kryptographie/slides/slides-5.1-anim.pdf}}
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    \item Alice wählt eine {\color{dunkelblau}Primzahl $p$} und {\color{dunkelblau}Erzeuger $g$ von $\mathbb{Z}_p$} mit $2 \leqslant g \leqslant p-2$
    \subitem $p$ und $g$ werden nicht geheim gehalten
    \item Alice wählt zufällige Zahl {\color{dunkelblau}$a\in[0,p-2]$} und berechnet {\color{dunkelblau}$A = g^a\ \mathrm{mod}\ p$}
    \subitem Alice hält {\color{dunkelblau}$a$ geheim} und schickt $(A, g, p)$ an $\rightarrow$ Bob
    \item Bob wählt zufällige Zahl {\color{dunkelblau}$b\in[0,p-2]$} und berechnet {\color{dunkelblau}$B = g^b\ \mathrm{mod}\ p$}
    \subitem Bob hält {\color{dunkelblau}$b$ geheim} und schickt $B$ an Alice
    \item Alice berechnet {\color{dunkelblau}$B^a\ \mathrm{mod}\ p = g^{a\cdot b}\ \mathrm{mod}\ p$}
    \item Bob berechnet {\color{dunkelblau}$A^b\ \mathrm{mod}\ p = g^{a\cdot b}\ \mathrm{mod}\ p$}
    \item Gemeinsamer Schlüssel {\color{dunkelblau}{\color{red}$K$}$= g^{a\cdot b}\ \mathrm{mod}\ p$} ist nur Alice und Bob bekannt, ohne das Alice $b$ kennt und Bob $a$
\end{itemize}
\begin{figure}[H]
    \begin{center}
...
\paragraph*{ElGamal-Verschlüsselung}ElGamal verwendet DH um einen asymmetrischen Verschlüsselungsalgorithmus zu erstellen. Daher ist es sicherer als RSA, welches `nur' Faktorisieren grosser Zahlen als unterliegende Funktion hat. Im Vergleich zu DH gibt es bei ElGamal weitere Teilschritte und feine Unterschiede. Die zu verschlüsselnde Nachricht wird in den \textsl{secret key} verschmelzt.

\subsubsection*{ElGamal-Verfahren im Detail\footnote{https://www.cs.uni-potsdam.de/ti/lehre/07-Kryptographie/slides/slides-5.2-anim.pdf}}\index{ElGamal}
\paragraph*{Schlüsselerzeugung}
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    \item Alice wählt eine {\color{dunkelblau}grosse Primzahl $p$} und ein {\color{dunkelblau}Gruppenelement $g$ der Ordnung $p$}
    \item Alice wählt eine {\color{dunkelblau}zufällige Zahl $a\in[0,p-2]$} und berechnet {\color{red}$A=g^a\ \mathrm{mod}\ p$}
    \subitem Alice hält {\color{dunkelblau}$a$ geheim} und schickt $(A, g, p)$ an $\rightarrow$ Bob
\end{itemize}
Bis hierhin verlief das Ganze ziemlich ähnlich zum DH-Schlüsselaustausch.
\paragraph*{Verschlüsselung}
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    \item Gesamtschlüssel ist {\color{red}$K=x\cdot g^{a\cdot b}\ \mathrm{mod}\ p$}
    \item Text wird in {\color{dunkelblau}Blöcke der Länge $\log_2 \frac{p}{8}$} zerlegt (ein Byte pro Buchstabe)
    \subitem Jeder {\color{dunkelblau}Textblock} wird als {\color{dunkelblau}Binärdarstellung einer Zahl $x$} interpretiert
    \item Bob wählt {\color{dunkelblau}zuälliges $b\in[0,p-2]$} und berechnet seinen öffentlichen Schlüssel {\color{dunkelblau}$B=g^b\ \mathrm{mod}\ p$}
    \item Bob verschlüsselt $x$ zu {\color{dunkelblau}$y=x\cdot A^b\ \mathrm{mod}\ p$}
    \item {\color{red}\textbf{TODO: WTF... literally... what the fucking fuck?! *fliptable}}
\end{itemize}
...
public class Randomness {
    ...
    public static void main(String[] args) {
        ...
        long i1 = 1767252209L;  //Random number 1
        long i2 = -1664600482L; //Random number 2
        long seed = 0;
...
Wenn mindestens die Hälfte der Teilnehmenden fair spielen, ist es nicht möglich, das Resultat oder die Teile der anderen Parteien herauszufinden. Das System kann umgangen werden, wenn jemand eine nichterlaubte Rechnung macht oder die letzte Person die ihr Resultat teilt ihre Antwort so anpasst das das resultierende Polynom ihre gewünschte Antwort ergibt.
...
\begin{figure}[H]
    \begin{center}
    \includegraphics[width=14cm]{images/Qubit0.png}
...
\begin{figure}[H]
    \begin{center}
    \includegraphics[width=14cm]{images/Qubit1.png}
...
\paragraph*{TODO}SAML, OAuth, OpenID, JWT
